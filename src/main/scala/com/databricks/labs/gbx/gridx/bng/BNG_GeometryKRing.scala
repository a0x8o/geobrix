...
object BNG_GeometryKRing extends WithExpressionInfo {
    ...
    def execute(geom: Geometry, res: Int, k: Int, mode: String): Set[String] = {
        val kRing = BNG.geometryKRing(geom, res, k, mode)
    ...
    override def builder(): FunctionBuilder = (c: Seq[Expression]) => c.length match {
        case 3 => new BNG_GeometryKRing(c(0), c(1), c(2), Literal(GeomDilation.DEFAULT_MODE))
        case 4 => new BNG_GeometryKRing(c(0), c(1), c(2), c(3))
    }
