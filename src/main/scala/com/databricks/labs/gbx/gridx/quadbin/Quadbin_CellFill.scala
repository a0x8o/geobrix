...
/** UDAF: `gbx_quadbin_cellfill(cellid, value, [k], [method], [power])`.
 *
 *  Grouped aggregator over a Quadbin `(cellid, value)` grid result: fills each NULL
...
case class Quadbin_CellFill(
    ...
    override def prettyName: String = Quadbin_CellFill.name

    override def children: Seq[Expression] = Seq(cellidExpr, valueExpr, kExpr, methodExpr, powerExpr)

    override protected def withNewChildrenInternal(nc: IndexedSeq[Expression]): Quadbin_CellFill =
    ...
    override def update(buffer: CellFillAcc, input: InternalRow): CellFillAcc = {
        val id = CellFill.parseCellIdLong(cellidExpr.eval(input), Quadbin_CellFill.name)
    ...
    override def eval(buffer: CellFillAcc): Any = {
        ...
        CellFill.toArrayData(Quadbin, CellFill.fill(Quadbin, buffer.cells.toSeq.toMap, k, method, power))
    }
...
/** Companion: SQL name gbx_quadbin_cellfill, builder (arities 2..5). */
object Quadbin_CellFill extends WithExpressionInfo {

    override def name: String = "gbx_quadbin_cellfill"

    override def builder(): FunctionBuilder = (c: Seq[Expression]) => c.length match {
        case 2 => Quadbin_CellFill(c(0), c(1), Literal(1), Literal("mean"), Literal(2.0))
        case 3 => Quadbin_CellFill(c(0), c(1), c(2),       Literal("mean"), Literal(2.0))
        case 4 => Quadbin_CellFill(c(0), c(1), c(2),       c(3),            Literal(2.0))
        case 5 => Quadbin_CellFill(c(0), c(1), c(2),       c(3),            c(4))
