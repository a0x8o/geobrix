...
/** Per-pixel/per-band MINIMUM of the valid (non-NoData) values across an ARRAY of aligned tiles.
...
case class RST_CombineMin(
    tiles: Expression
) extends InvokedExpression {

    private def rasterType = RST_ExpressionUtil.arrayOfTileRasterType(RST_CombineMin.name, tiles)
    ...
    override def prettyName: String = RST_CombineMin.name
    override def replacement: Expression = invoke(RST_CombineMin)
...
object RST_CombineMin extends WithExpressionInfo {
    ...
    def execute(tiles: Seq[(Long, Dataset, Map[String, String])]): (Long, Dataset, Map[String, String]) = {
        ...
        val (combinedRaster, mtd) = CombineStats.compute(tiles.map(_._2).toArray, tiles.head._3, "min")
        (cellID, combinedRaster, mtd)
    }

    override def name: String = "gbx_rst_combinemin"

    override def builder(): FunctionBuilder = (c: Seq[Expression]) => new RST_CombineMin(c(0))
