...
case class RST_BNG_RasterToGridMin(
    tile: Expression,
...
object RST_BNG_RasterToGridMin extends WithExpressionInfo {
    ...
    /** Centroid reducer: minimum pixel value. */
    val fAgg: ArrayBuffer[Double] => Double = values => values.min
    /** Covering reducer: minimum value across contributing pixels (weight-agnostic). */
    ...
    override def builder(): FunctionBuilder = (c: Seq[Expression]) => c.length match {
        case 2 => new RST_BNG_RasterToGridMin(c(0), c(1), Literal("complete"), Literal("centroid"))
        case 3 => new RST_BNG_RasterToGridMin(c(0), c(1), c(2), Literal("centroid"))
        case 4 => new RST_BNG_RasterToGridMin(c(0), c(1), c(2), c(3))
