...
case class RST_BNG_RasterToGridAvg(
    tile: Expression,
...
object RST_BNG_RasterToGridAvg extends WithExpressionInfo {
    ...
    /** Centroid reducer: arithmetic mean of the cell's pixel values. */
    val fAgg: ArrayBuffer[Double] => Double = values => values.sum / values.length
    /** Covering reducer: area-weighted mean, `Σ(v*w) / Σw`. */
    ...
    /** Canonical Stage-2 path: coverage/assignment-aware, nullable-measure output. */
    ...
    /** Legacy sparse+centroid shim (pre-Stage-2 return type) retained for existing direct-execute
      * callers (bench/baseline tests). Emits only has-data cells; never a null measure. */
    ...
    override def builder(): FunctionBuilder = (c: Seq[Expression]) => c.length match {
        case 2 => new RST_BNG_RasterToGridAvg(c(0), c(1), Literal("complete"), Literal("centroid"))
        case 3 => new RST_BNG_RasterToGridAvg(c(0), c(1), c(2), Literal("centroid"))
        case 4 => new RST_BNG_RasterToGridAvg(c(0), c(1), c(2), c(3))
