...
object RST_Quadbin_RasterToGridAvg extends WithExpressionInfo {
    ...
    /** Legacy sparse+centroid shim (pre-Stage-2 return type) retained for existing direct-execute
      * callers (integration/bench/baseline tests). Emits only has-data cells; never a null measure. */
    ...
    override def builder(): FunctionBuilder = (c: Seq[Expression]) => c.length match {
        case 2 => new RST_Quadbin_RasterToGridAvg(c(0), c(1), Literal("complete"), Literal("centroid"))
        case 3 => new RST_Quadbin_RasterToGridAvg(c(0), c(1), c(2), Literal("centroid"))
        case 4 => new RST_Quadbin_RasterToGridAvg(c(0), c(1), c(2), c(3))
