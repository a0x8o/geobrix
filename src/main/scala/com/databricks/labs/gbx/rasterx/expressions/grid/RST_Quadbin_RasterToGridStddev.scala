...
/** Returns the population standard deviation of raster values within each quadbin grid cell. */
...
object RST_Quadbin_RasterToGridStddev extends WithExpressionInfo {
    ...
    /** Centroid reducer: population stddev = sqrt(population variance), two-pass (see spec 3.2). */
    ...
    /** Covering reducer: sqrt of the weighted population variance (reuses the variance reducer). */
    val fAggW: ArrayBuffer[(Double, Double)] => Double = pairs =>
        math.sqrt(RST_Quadbin_RasterToGridVariance.fAggW(pairs))
    ...
    override def builder(): FunctionBuilder = (c: Seq[Expression]) => c.length match {
        case 2 => new RST_Quadbin_RasterToGridStddev(c(0), c(1), Literal("complete"), Literal("centroid"))
        case 3 => new RST_Quadbin_RasterToGridStddev(c(0), c(1), c(2), Literal("centroid"))
        case 4 => new RST_Quadbin_RasterToGridStddev(c(0), c(1), c(2), c(3))
