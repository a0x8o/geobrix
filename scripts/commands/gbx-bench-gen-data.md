...
**Options:** `--out <dir>`, `--tile-px <list>`, `--bands <list>`, `--dtypes <list>`, `--srids <list>`, `--nodata-frac <list>`, `--row-rows <n>`, `--seed <n>`, `--set core|full` (accepted for pipeline symmetry; the corpus is function-agnostic, so it does not change corpus content), `--log <path>`, `--help`.
