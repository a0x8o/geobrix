...
**Prerequisites (operator):** a provisioned cluster + filled `notebooks/tests/databricks_cluster_config.env`. Heavyweight needs an x86 DBR 17.3 LTS cluster with the init script + bundle + geobrix wheel + the bench `geobrix-*-tests.jar` staged; lightweight (incl. ARM) needs just the `[pyrx]` wheel. On ARM clusters use `--lightweight-only` (heavyweight is x86-only by design).
