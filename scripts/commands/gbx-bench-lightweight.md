...
**Options:** `--corpus <dir>`, `--out <path>`, `--run-id <id>`, `--functions <list>`, `--categories <list>`, `--set core|full` (selection tier; default `core`, overridden by an explicit `--functions`), `--mode pure-core|spark-path|both`, `--row-counts <list>`, `--warmup <n>`, `--measured <n>`, `--driver-mem <m>` (Spark driver heap for the spark-path leg, default 4g), `--log <path>`, `--help`.
...
- `bash scripts/commands/gbx-bench-lightweight.sh --set full --mode pure-core`
