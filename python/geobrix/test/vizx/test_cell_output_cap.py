...
import databricks.labs.gbx.vizx._interactive as I
...
def test_raise_cap_fires_magic_when_registered(monkeypatch):
    ...
    monkeypatch.setattr(I, "get_ipython", lambda: ip)
    assert I._raise_cell_output_cap() is True
...
def test_raise_cap_skips_when_magic_absent(monkeypatch):
    ...
    monkeypatch.setattr(I, "get_ipython", lambda: ip)
    assert I._raise_cell_output_cap() is False
    assert ip.calls == []


def test_raise_cap_skips_without_ipython(monkeypatch):
    monkeypatch.setattr(I, "get_ipython", lambda: None)
    assert I._raise_cell_output_cap() is False
...
def test_raise_cap_never_raises(monkeypatch):
    ...
    monkeypatch.setattr(I, "get_ipython", boom)
    assert I._raise_cell_output_cap() is False  # graceful, no exception
...
def test_plot_interactive_raises_cap_on_interactive_when_enabled(monkeypatch):
    calls = []
    monkeypatch.setattr(I, "_raise_cell_output_cap", lambda: (calls.append(1) or True))
    ...
    monkeypatch.setattr(I, "_notebook_display_html", lambda: None)  # -> return html str

    out = I.plot_interactive(_LYRS, set_cell_max_output=True)
...
def test_plot_interactive_skips_cap_when_disabled(monkeypatch):
    ...
    monkeypatch.setattr(M, "prepare_layers", lambda *a, **k: _result("interactive"))
    monkeypatch.setattr(M, "build_html", lambda *a, **k: "<div>map</div>")
    monkeypatch.setattr(I, "_notebook_display_html", lambda: None)

    I.plot_interactive(_LYRS, set_cell_max_output=False)
