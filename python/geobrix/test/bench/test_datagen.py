...
def test_generate_corpus_writes_tiles_and_manifest(tmp_path):
    corpus = dg.generate_corpus(
        out_dir=tmp_path, seed=11,
        tile_px=[32, 64], bands=[1, 2], dtypes=["float32"],
        srids=[4326, 3857], nodata_fracs=[0.0, 0.1],
        row_rows=20, row_tile_px=64, row_bands=2, row_dtype="float32",
    )
...
def test_validity_gate_passes_for_generated_corpus(tmp_path):
    corpus = dg.generate_corpus(
        out_dir=tmp_path, seed=5,
        tile_px=[32], bands=[1], dtypes=["float32"],
        srids=[4326], nodata_fracs=[0.02], row_rows=3,
        row_tile_px=32, row_bands=1, row_dtype="float32",
    )
