...
def dilate(frontier0, visited0, neighbors, admit):
    ...
    while frontier:
        k += 1
        nxt = {n for c in frontier for n in neighbors(c) if n not in visited and admit(n)}
        if not nxt:
            return
        visited |= nxt
        frontier = nxt
...
def mode_setup(mode, cls):
    ...
    if mode == "boundary-out":
        return frozenset(pb), frozenset(cls.p_cover), (lambda n: True), frozenset(cls.p_cover)
    if mode == "boundary-in":
    ...
    if mode == "hole-out":
        return frozenset(hb), frozenset(cls.h_cover), (lambda n: n in cls.p_core), frozenset(hb)
    # hole-out-ignore-geom
    return frozenset(hb), frozenset(cls.h_cover), (lambda n: n not in cls.h_core), frozenset(hb)
