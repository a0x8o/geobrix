...
def classify(geom, res):
    ...
    is set to the union of those cells (p_cover = s_cover = cells; core = empty since
    no polygon cell can be contained by a point or line).
    """
    dim = _dilate._geom_dimension(geom)

    if dim != 2:
        # Non-polygon: derive covering cells from coordinate samples.
        # A point/line has no 2D interior → p_core = s_core = h_cover = h_core = {}
