...
- **`gbx_rst_h3_tessellate` now includes fringe cells that clip the raster edge (behavior change since v0.3.0).** Candidate H3 cells were previously selected with a centroid-containment fill of the raster's bounding box, so an edge cell whose hexagon overlaps the raster but whose center falls just outside the bounding box was omitted — leaving a thin uncovered fringe along the raster border. Candidate selection now also considers the immediate neighbors of every covering cell, so any cell whose hexagon genuinely clips the raster is included; cells that do not actually overlap are still dropped. As a result, edge tiles may return a few additional H3 cells. Interior coverage and the per-cell clip are unchanged. See [Raster Functions](./api/raster-functions).
