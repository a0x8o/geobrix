...
Augments Databricks built-in `ST_*` functions with vector-tile encoding and legacy-Mosaic migration helpers.
...
**Use VectorX when:** encoding features as Mapbox Vector Tiles; generating per-tile MVT layers; reading vector formats (Shapefile, GeoJSON, GeoPackage, FileGDB); migrating from DBLabs Mosaic.
