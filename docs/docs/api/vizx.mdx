---
sidebar_position: 11
title: VizX
---

import VizXIcon from '../../../resources/images/brand/VizX.png';

# <img src={VizXIcon} alt="VizX" style={{height: '3em', width: 'auto', verticalAlign: 'middle', marginRight: '0.5rem'}} /> VizX

`databricks.labs.gbx.vizx` renders rasters and turns Spark DataFrames into GeoDataFrames for interactive maps. It is **tier-agnostic** — the raster plotters take tile bytes (use them with `pyrx` *or* `rasterx` tiles), and the vector adapters take any Spark DataFrame.

These are driver-side, single-node helpers for inspecting results in a notebook — not distributed operations. They collect to the driver, so the vector adapters guard the collect with `max_rows`.

<figure>
  <img src="../../../resources/images/diagrams/vizx/screenshots/vizx-interactive-hero.png" alt="A multi-layer interactive map (buildings + H3 grid) rendered by plot_interactive — self-contained MapLibre, no tile server." style={{width: '100%'}} />
  <figcaption>A multi-layer interactive map (buildings + H3 grid) rendered by <code>plot_interactive</code> — self-contained MapLibre, no tile server.</figcaption>
</figure>

:::note Opt-in extra
`gbx.vizx` requires `geobrix[vizx]`, which pulls in `matplotlib`, `geopandas`, `folium`, and `mapclassify`. The package itself imports only `matplotlib` (raster rendering) and `geopandas` (the GeoDataFrame adapters); `folium` + `mapclassify` are used by `plot_interactive` / `GeoDataFrame.explore()` for interactive maps.
:::

## Installation

```bash
pip install "geobrix[vizx]"
```

From a Databricks notebook (commonly alongside the lightweight tier):

```python
%pip install --quiet "geobrix[light,vizx] @ file:///Volumes/<catalog>/<schema>/<volume>/geobrix-0.4.0-py3-none-any.whl"
```

## Import

```python
from databricks.labs.gbx.vizx import (
    plot_raster,
    plot_file,
    plot_mask_layers,
    plot_static,
    plot_interactive,
    plot_pmtiles,
    plot_cog,
    as_gdf,
    cells_as_gdf,
    grid_as_gdf,
)
from databricks.labs.gbx.pmtiles import pmtiles_info
```

## What's in VizX

| Sub-page | What it covers | When to use it |
|---|---|---|
| [Multi-Layer Compositor](./vizx-layers) | `plot_static` / `plot_interactive` with multiple layers; embed-size ladder; audit; simplify | Overlaying vector, raster, grid, and PMTiles layers in one map |
| [PMTiles Viewers](./vizx-pmtiles) | `pmtiles_info`, `plot_pmtiles`, `plot_cog` | Inspecting and rendering PMTiles archives and COGs inline |
| [Rendering & Adapters](./vizx-rendering) | `plot_raster`, `plot_file`, `plot_mask_layers`, `as_gdf`, `cells_as_gdf`, `grid_as_gdf`, `plot_static` (single-layer), `plot_interactive` (single-layer), escape hatches | Rendering raster tiles, converting DataFrames to GeoDataFrames, single-layer maps |

## Next steps

- [Multi-Layer Compositor](./vizx-layers) — layer types, embed-size ladder, ephemeral/durable simplify, scale guidance
- [PMTiles Viewers](./vizx-pmtiles) — `pmtiles_info`, `plot_pmtiles`, `plot_cog`
- [Rendering & Adapters](./vizx-rendering) — raster rendering, vector adapters, single-layer maps, escape hatches
- [RasterX Function Reference](./raster-functions)
- [STAC Client](./stac)
- [Helios notebooks](../notebooks/helios) — worked end-to-end example using `plot_pmtiles`, `plot_cog`, and `pmtiles_info` to render San Francisco tilesets inline.
