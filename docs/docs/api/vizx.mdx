...
| `'custom'` | Planned — raises `NotImplementedError` (a custom grid needs its grid config). |

Cell boundaries reuse the lightweight GridX cell→geometry implementations, so
they match `gbx_h3_*`/`gbx_quadbin_aswkb`/`gbx_bng_aswkb` exactly. Each grid's
native CRS is honoured (BNG is metres in EPSG:27700) and reprojected to Web
Mercator for the basemap.
