...
These helpers render a `.pmtiles` archive or a Cloud-Optimized GeoTIFF in a notebook — driver-side, no tile server, no distributed op.
...
| `**map_kwargs` | | | Forwarded to the static plotter (`plot_raster` or `plot_static`). |
