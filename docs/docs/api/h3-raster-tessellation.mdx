...
`rst_h3_tessellate(tile, resolution, assignment, coverage)` has two optional parameters. `assignment` (3rd arg, defaults to `'centroid'`) controls how pixels are distributed across cells; `coverage` (4th arg, defaults to `'complete'`) controls whether empty-but-covered cells are emitted.

| | `assignment='covering'` | `assignment='centroid'` (default) |
|---|---|---|
...
Use **`assignment='centroid'`** (default) when:
...
Both assignment values use `LATERAL` (streaming UDTF). The two-argument form uses defaults (`assignment='centroid'`, `coverage='complete'`); pass the third argument to select an assignment, and the fourth for coverage.

```sql
-- covering assignment — every overlapping cell, each clipped to its hexagon
SELECT t.*
FROM rasters,
LATERAL gbx_rst_h3_tessellate(tile, 7, 'covering') t;

-- centroid assignment (default) — pixel-centroid single-assignment partition
SELECT t.*
FROM rasters,
LATERAL gbx_rst_h3_tessellate(tile, 7, 'centroid') t;

-- two-argument form: defaults to assignment='centroid', coverage='complete'
SELECT t.*
FROM rasters,
LATERAL gbx_rst_h3_tessellate(tile, 7) t;

-- four-argument explicit form
SELECT t.*
FROM rasters,
LATERAL gbx_rst_h3_tessellate(tile, 7, 'centroid', 'complete') t;
```
...
# centroid assignment (default)
chips = df.select(rx.rst_h3_tessellate("tile", 7))
chips = df.select(rx.rst_h3_tessellate("tile", 7, assignment="centroid"))

# covering assignment
chips = df.select(rx.rst_h3_tessellate("tile", 7, assignment="covering"))
```
...
The five `rst_h3_rastertogrid*` functions (`avg`, `count`, `max`, `min`, `median`) use the **same pixel-centroid assignment rule** as `assignment='centroid'` (the default). The difference is the output:
...
```sql
-- chip-emitting variant (centroid assignment — default)
SELECT t.*
FROM rasters,
LATERAL gbx_rst_h3_tessellate(tile, 7, 'centroid') t;
...
Both tiers compute the same cell sets and the same per-cell chip pixels for each assignment:

- **`assignment='covering'`:** lightweight (`pyrx`) calls h3-py 4.x `polygon_to_cells_experimental(..., contain='overlap')`; heavyweight (`rasterx`) hand-rolls the equivalent covering set using JTS geometry intersection (H3-Java is pinned at 3.7.0, which predates the v4 covering primitive). The mechanisms differ, but the output is the same overlapping set — verified by the cross-tier parity test. On top of that, each tier emits the cells that clip to all-NoData (rather than dropping them) when `coverage='complete'` and reduces such a chip to SQL `NULL` — verified by an all-NoData regression test on each tier.
- **`assignment='centroid'`** (default)**:** both tiers assign each pixel via its centroid lat/lon to a single H3 cell using the same point-in-hexagon lookup; the chip pixels are identical by construction.

Parity is test-enforced: for each assignment, tests assert that light and heavy produce the same cell set and the same per-cell chip pixels on a border-containing tile.
