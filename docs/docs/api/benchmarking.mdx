...
`gbx_pmtiles_agg` is the grouped-aggregate companion to the PMTiles writer: it
...
in-memory archive — the same execution shape as `gbx_st_asmvt`.
...
diverge. Tiles compose with `gbx_pmtiles_agg` for end-to-end publishing.

### TIN & legacy migration (`st_triangulate`, `st_interpolateelevation*`, `st_legacyaswkb`)

The constrained-Delaunay TIN generators (`gbx_st_triangulate`, `gbx_st_interpolateelevationbbox`,
`gbx_st_interpolateelevationgeom`) and the legacy-geometry migration function (`gbx_st_legacyaswkb`)
...
|---|---|---|---|---|
| `gbx_st_interpolateelevationgeom` | 125 | 0.49 | 0.56 | 0.88× |
| `gbx_st_legacyaswkb` | 1,000 | 0.39 | 0.48 | 0.82× |
| `gbx_st_interpolateelevationbbox` | 245 | 0.40 | 0.53 | 0.75× |
| `gbx_st_triangulate` | 220 | 0.41 | 0.70 | 0.60× |
...
parity across four representative shapes: a scalar encode (`gbx_quadbin_pointascell`, lon/lat → cell),
a geometry → cell-array fill (`gbx_quadbin_polyfill`), a geometry → cell-clip struct array
(`gbx_quadbin_tessellate`), and a grouped aggregate (`gbx_quadbin_cellunion_agg`, cell ids → unioned
coverage geometry). Both tiers expose the same `gbx_quadbin_*` SQL names, so the light tier is
...
All 10 quadbin functions were benchmarked on a cluster (1,000 rows, both tiers). **Exact cell-set
parity holds for all 10** (test-enforced — in-cell and by the `test_parity_quadbin` cross-tier suite).

On *timing*, these are **sub-millisecond cell-math** operations, so the spark-path wall-clock is
dominated by per-row JVM↔Python dispatch and **cluster run-to-run variance**, not by the work itself —
the same (unchanged) function varied by ~±40% between repeated 5-iteration runs. Precise per-function
speedups therefore do not reproduce and aren't meaningful to publish at this granularity; across runs
the lightweight tier sits **roughly at parity** with the heavyweight JVM tier on the whole quadbin
surface (sometimes faster, sometimes modestly slower, within that variance).
...
**All 23 `gbx_bng_*` functions were benchmarked on a cluster** (DBR 17.3 LTS, spark-path, 1,000
...
|---|---|---|---|
| `gbx_bng_kring` | 0.20 | 0.17 | 1.18× |
| `gbx_bng_geomkring` | 0.15 | 0.13 | 1.11× |
| `gbx_bng_geomkringexplode` | 0.48 | 0.44 | 1.08× |
| `gbx_bng_cellunion_agg` | 0.29 | 0.27 | 1.07× |
| `gbx_bng_aswkt` | 0.18 | 0.17 | 1.06× |
| `gbx_bng_centroid` | 0.14 | 0.14 | 1.06× |
| `gbx_bng_kloop` | 0.13 | 0.13 | 1.01× |
| `gbx_bng_cellunion` | 0.14 | 0.15 | 0.97× |
| `gbx_bng_aswkb` | 0.14 | 0.14 | 0.96× |
| `gbx_bng_kloopexplode` | 0.26 | 0.27 | 0.96× |
| `gbx_bng_kringexplode` | 0.26 | 0.27 | 0.96× |
| `gbx_bng_cellarea` | 0.15 | 0.16 | 0.94× |
| `gbx_bng_geomkloopexplode` | 0.43 | 0.47 | 0.93× |
| `gbx_bng_euclideandistance` | 0.14 | 0.15 | 0.91× |
| `gbx_bng_geomkloop` | 0.15 | 0.16 | 0.90× |
| `gbx_bng_polyfill` | 0.14 | 0.16 | 0.89× |
| `gbx_bng_tessellateexplode` | 0.42 | 0.47 | 0.89× |
| `gbx_bng_cellintersection_agg` | 0.25 | 0.28 | 0.89× |
| `gbx_bng_eastnorthasbng` | 0.14 | 0.16 | 0.88× |
| `gbx_bng_tessellate` | 0.14 | 0.17 | 0.86× |
| `gbx_bng_pointascell` | 0.15 | 0.17 | 0.86× |
| `gbx_bng_distance` | 0.13 | 0.16 | 0.86× |
| `gbx_bng_cellintersection` | 0.14 | 0.17 | 0.82× |

### Custom grid (all 7 `gbx_custom_*` functions)
...
**All 7 `gbx_custom_*` functions were benchmarked on a cluster** (DBR 17.3 LTS, x86, spark-path,
1,000 tiles/iteration, both tiers). **Exact cell-id/cell-set parity holds for all 7** — light output
equals heavy output (cell ids/sets exact; geometry within 1e-6; grid struct identical), enforced by
the JAR-gated cross-tier parity suite and the in-cell hard-assert gates on the cluster run.

On *timing*, the pure-Python `pygx` custom-grid tier is **competitive on the cheapest op**
(`custom_centroid` ~1.06×) and **~0.5–0.8× on the rest**. Absolute times are all sub-second at 1,000
tiles (~0.0002–0.0007 s/tile), so the gap is the Python UDF-boundary / Arrow serialization fixed cost
dominating very cheap per-row grid math (the heavyweight Scala tier has near-zero per-row cost) — not
an algorithmic deficiency. As with the byte-heavy raster ops, the per-row boundary tax, not the
compute, sets the at-scale cost for these sub-millisecond operations.
...
| Function | Heavy (s) | Light (s) | Light speedup |
|---|---|---|---|
| `gbx_custom_centroid` | 0.24 | 0.23 | 1.06× |
| `gbx_custom_polyfill` | 0.27 | 0.34 | 0.79× |
| `gbx_custom_cellaswkb` | 0.26 | 0.34 | 0.77× |
| `gbx_custom_kring` | 0.23 | 0.39 | 0.60× |
| `gbx_custom_cellaswkt` | 0.26 | 0.48 | 0.55× |
| `gbx_custom_pointascell` | 0.35 | 0.64 | 0.55× |
| `gbx_custom_grid` | 0.37 | 0.71 | 0.53× |
