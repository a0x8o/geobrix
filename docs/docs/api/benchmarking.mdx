...
| **GeoJSONL (sharded, `geojsonl_gbx`)** | 14M | **16.4 s** (80 shards) | **~855k rows/s** |
...
per-feature). The **sharded `geojsonl_gbx`** writer skips the merge entirely — one
newline-delimited shard per partition, written in parallel — so it is **~17× faster** than the
single-file GeoJSON writer at 14M (16.4 s vs 279 s) and scales with partitions. It's the
recommended writer for large / any-scale output; `maxRecordsPerFile` caps features per shard.
