...
|---|---|---|---|---|
| `pmtiles_agg` | lightweight | 0.74 s | 0.74 ms | — |
| `pmtiles_agg` | heavyweight | 0.70 s | 0.70 ms | **~0.94×** (≈ parity) |

The two tiers run within **~6%** of each other (light min 0.65 s overlaps heavy p90
0.76 s), so the gap sits inside the run-to-run noise band for a grouped aggregate of
this size. Both tiers produce **spec-valid PMTiles v3 archives** that decode to the
same tile set; both write the directory with `internal_compression = none` for
v0.4.0, so the archives are decode-identical. The lightweight aggregator is a
