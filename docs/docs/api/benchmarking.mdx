...
|---|---|---|---|
| GeoJSON | 14M | 279.1 s | ~50k rows/s |
| Shapefile | 14M | 152.6 s | ~92k rows/s |
| GeoPackage | 14M | 155.9 s | ~90k rows/s |
| FileGDB | 14M | 368.3 s | ~38k rows/s |

The single-file write is driver-bound (one merged output); GeoJSON is the slowest (text
encoding) and FileGDB next (native `osgeo`, per-feature). For sharded, parallel output that
skips the driver merge, write a directory of newline-delimited shards (`geojsonl_gbx`, one
file per partition).
