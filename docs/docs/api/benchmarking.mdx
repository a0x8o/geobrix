...
- **[Geometry-aware expansion scales linearly](#results)** — `geomkring`/`geomkloop` grow grid rings from a geometry: on Serverless, 10,000 notional building footprints expand to h3 res-10 rings (`k=10`) in ~6.3 s (~1,600 buildings/s), and 100,000 footprints reach 34.6M cells in ~47 s. h3 leads the four grids; see the Grid tab.
...
### Geometry-aware expansion at scale (`geomkring` / `geomkloop`)
...
**Headline** — 10,000 building footprints, h3 res 10, `geomkring` `boundary-out`, `k=10`: **≈ 6.3 s** producing **3,470,960 cells** — about **1,600 buildings/s** and **550k cells/s** on Serverless.

**Across grids** (10,000 buildings, `k=10`, `geomkring`):

| Grid | Resolution (≈65 m cell) | Wall (s) | Cells | Buildings/s | Cells/s |
|---|---|---|---|---|---|
| h3 | res 10 | **6.3** | 3,470,960 | 1,589 | 551k |
| custom | 65 m root (EPSG:27700) | 13.2 | 3,651,174 | 758 | 277k |
| quadbin | res 19 | 41.6 | 4,951,123 | 240 | 119k |
| bng | res 4 (100 m) | 53.2 | 4,762,643 | 188 | 90k |

h3 is fastest — its neighbour step is pure integer index math (`h3` grid-disk topology) and the geometry work is offloaded to Databricks-native `h3_coverash3`/`h3_polyfillash3`. custom is a close second (a pure-Python lattice walk). quadbin (web-mercator `(z,x,y)` math + EWKB) and BNG (STRING cell ids + chip geometry) carry more per-cell work; BNG's res 4 also emits coarser 100 m cells. Cell sizes and CRS differ per grid, so read the ordering rather than a direct cell-for-cell comparison.
...
|---|---|---|---|---|
| 1 | 1.65 | 108,554 | 1.63 | 79,803 |
| 5 | 3.25 | 1,019,705 | 3.02 | 315,327 |
| 10 | 6.3 | 3,470,960 | 6.01 | 606,867 |

The filled ring's cell count grows ~`k²` (disk area) while the hollow loop grows ~`k` (perimeter), so at `k=10` the loop emits ~6× fewer cells; wall-clock tracks the covering-set frontier work, which is similar for both.
...
|---|---|---|---|
| 100 | 0.99 | 35,053 | 101 |
| 1,000 | 2.08 | 349,332 | 481 |
| 10,000 | 6.3 | 3,470,960 | 1,589 |
| 100,000 | 47.4 | 34,638,156 | 2,111 |

Throughput scales ~linearly at volume: **100,000 building footprints expand to 34.6M cells in ≈ 47 s**. Buildings/s climbs with `N` as the fixed job overhead amortizes, plateauing around 2,000 buildings/s.

**Hole modes at scale** — on courtyard footprints (buildings with an interior hole), `hole-in` `k=5` over 1,000 buildings emits 1,768 cells, expanding inward from each hole's edge — the hole-fill traversals work on holed geometries at scale, not just in unit tests.
