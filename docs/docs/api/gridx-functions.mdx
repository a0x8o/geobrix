...
Geometry-aware functions polyfill a geometry into the grid and then expand outward (or inward at holes) using the dilation engine. They generalise the cell-centric `quadbin_kring` / `quadbin_kloop` to operate on arbitrary geometry shapes. The optional `mode` parameter selects the expansion direction and controls how polygon holes are handled; `'boundary-out'` (the default) expands outward from the covering set and works correctly on aligned and non-aligned geometries alike.

**Dilation modes** (exact hyphenated values):

| Mode | Behaviour |
|------|-----------|
| `'boundary-out'` | Default. Expands outward from the geometry's covering set. Polygon holes do not act as boundaries. |
| `'boundary-in'` | Expands inward from the outer edge of the covering set, stopping at holes. |
| `'boundary-in-ignore-holes'` | Like `'boundary-in'` but treats the polygon as solid — expands inward across holes. |
