...
**Dilation modes** work the same as for BNG/quadbin/custom, and all six operate on holed geometries: `'hole-in'`, `'hole-out'`, and `'hole-out-ignore-geom'` expand from the extracted holes; `'boundary-in-ignore-holes'` treats the polygon as solid, expanding inward across holes as if they were filled (the Python API polyfills the solid directly for an exact inward band).
