...
GridX is **partially lightweight** as of v0.4.0: the **quadbin** functions (`gbx_quadbin_*`) run in both tiers <Tier both/>, while the **BNG** (`gbx_bng_*`) and **custom-grid** (`gbx_custom_*`) functions are heavyweight-only <Tier heavy/>. Each section below is marked with its tier. See [Choosing an Execution Tier](./execution-tiers) for the lightweight vs heavyweight comparison.
...
<Tier heavy/> BNG functions (`gbx_bng_*`) are heavyweight-only.
...
In SQL, GridX functions are prefixed with **`gbx_`** (e.g. `gbx_bng_aswkb`, `gbx_quadbin_pointascell`). For more language-specific tips, see the [Python](./python), [Scala](./scala), and [SQL](./sql) API pages.
:::
...
**Scalar UDF** (returns `ARRAY<STRING>`)
...
**Scalar UDF** (returns `ARRAY<STRUCT>`)
...
<Tier heavy/> Custom-grid functions (`gbx_custom_*`) are heavyweight-only.
...
<Tier both/> Available in both tiers (lightweight `pygx` + heavyweight Scala), with exact cell-set parity enforced by the cross-tier parity suite.
...
**Scalar UDF** (returns `BINARY` — EWKB polygon, SRID 4326)
...
**Scalar UDF** (returns `BINARY` — EWKB point, SRID 4326)
...
**Scalar UDF** (returns `ARRAY<STRUCT<cell, geom>>`)
...
**Scalar UDF** (`ARRAY<LONG>` → EWKB MultiPolygon)
...
**Grouped-aggregate UDF** (returns EWKB MultiPolygon)
