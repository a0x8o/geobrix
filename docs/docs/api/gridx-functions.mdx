...
Geometry-aware functions polyfill a geometry into the grid and then expand outward (or inward at holes) using the dilation engine. They generalise the cell-centric `quadbin_kring` / `quadbin_kloop` to operate on arbitrary geometry shapes. The optional `mode` parameter controls how cells that touch the geometry boundary are classified; `'boundary-out'` (the default) keeps the covering boundary cells and expands outward.

**Dilation modes** (exact hyphenated values):

| Mode | Behaviour |
|------|-----------|
| `'boundary-out'` | Default. Boundary cells included; expansion grows outward from the edge. |
| `'boundary-in'` | Boundary cells included; expansion grows inward into the geometry. |
| `'boundary-in-ignore-holes'` | Like `'boundary-in'` but hole boundaries are not subtracted. |
| `'hole-in'` | Expands inward from the interior edge of holes only. |
| `'hole-out'` | Expands outward from hole boundaries into the surrounding geometry. |
| `'hole-out-ignore-geom'` | Expands outward from holes ignoring the outer geometry boundary. |
...
**Signature:** `quadbin_geomkring(geom, resolution, k, [mode])`
...
- `mode` *(optional)* — boundary classification mode (default `'boundary-out'`). See table above.
...
**Signature:** `quadbin_geomkloop(geom, resolution, k, [mode])`
...
**Signature:** `quadbin_geomkringexplode(geom, resolution, k, [mode])`
...
**Signature:** `quadbin_geomkloopexplode(geom, resolution, k, [mode])`
...
**Signature:** `bng_geomkring(geom, resolution, k, [mode])`

**Parameters:**
- `geom` - Input geometry (WKT or WKB, EPSG:27700 coordinates required)
- `resolution` - BNG resolution: integer index (e.g. 3 for 1 km) or string (e.g. `'1km'`, `'100m'`)
- `k` - Integer ring distance
- `mode` *(optional)* — boundary classification mode (default `'boundary-out'`). Controls how cells touching the geometry edge are treated. Values: `'boundary-out'`, `'boundary-in'`, `'boundary-in-ignore-holes'`, `'hole-in'`, `'hole-out'`, `'hole-out-ignore-geom'`.
...
**Signature:** `bng_geomkloop(geom, resolution, k, [mode])`

**Parameters:**
- `geom` - Input geometry (WKT or WKB, EPSG:27700 coordinates required)
- `resolution` - BNG resolution: integer index (e.g. 3 for 1 km) or string (e.g. `'1km'`, `'100m'`)
- `k` - Integer ring distance
- `mode` *(optional)* — boundary classification mode (default `'boundary-out'`). See `bng_geomkring` for the full list of values.
...
**Signature:** `bng_geomkringexplode(geom, resolution, k, [mode])`
...
**Signature:** `bng_geomkloopexplode(geom, resolution, k, [mode])`
...
:::note Boundary alignment
When the geometry edges fall exactly on cell boundaries, all cells in the polyfill may be classified as interior (no boundary cells), making the `boundary-out` frontier empty and the k=1 ring identical to the k=0 covering set. This occurs for polygons whose coordinates are exact multiples of the grid's cell size at the requested resolution. Use geometries with sub-cell offsets, or a finer resolution, to get outward expansion.
:::
...
**Signature:** `custom_geomkring(geom, grid, resolution, k, [mode])`
...
- `k` — Ring distance (0 = covering set only)
- `mode` *(optional)* — boundary classification mode (default `'boundary-out'`). See Quadbin section for values.
...
**Signature:** `custom_geomkloop(geom, grid, resolution, k, [mode])`
...
**Signature:** `custom_geomkringexplode(geom, grid, resolution, k, [mode])`
...
**Signature:** `custom_geomkloopexplode(geom, grid, resolution, k, [mode])`
...
H3 geometry-aware kring/kloop functions are **light-only** — there is no heavy (Scala) implementation. They are designed for Serverless compute on Databricks, where the geometry-to-cell decomposition is done by Databricks product H3 functions (`h3_coverash3`, `h3_polyfillash3`). The topological expansion step (the dilation engine) runs as a pure-Python UDF registered by the `pygx` light tier.

:::note Architecture
The Python column API (`from databricks.labs.gbx.gridx.h3.functions import geomkring`) composes the two steps automatically: it calls the product H3 functions columnar to compute cover/core cell arrays, then passes those arrays to the `gbx_h3_geomkring` UDF. The SQL functions (`gbx_h3_geomkring` etc.) take the pre-computed arrays directly.

For hole-reading modes the Python column API also extracts the geometry's holes columnar — `ST_Difference(ST_MakePolygon(ST_ExteriorRing(geom)), geom)` polyfilled by the product H3 functions — so the geometry input for those modes must be WKB `BINARY` (the canonical form); `'boundary-out'` and `'boundary-in'` additionally accept WKT `STRING`.
:::

**Dilation modes** work the same as for BNG/quadbin/custom, and all six operate on holed geometries: `'hole-in'`, `'hole-out'`, and `'hole-out-ignore-geom'` expand from the extracted holes. `'boundary-in-ignore-holes'` treats the polygon as solid; on holed geometries its inward band may leave small notches at hole rims (it reconstructs the solid-fill core from the hole cells rather than polyfilling the solid directly) — exact for geometries whose hole edges align to cell boundaries.

### h3_geomkring

<Tier light/>

Geometry-aware H3 k-ring. Computes the covering set via product H3 functions then expands by `k` ring steps using H3 neighbour topology.

**Signature:** `h3_geomkring(cover, core, holes_cover, holes_core, solid_core, k, [mode])`

**SQL surface (direct):** Takes pre-computed `ARRAY<BIGINT>` cover/core arrays.

**Python API (recommended on Databricks):**
```python
...
**Parameters:**
- `cover` — `ARRAY<BIGINT>` cells overlapping the geometry (from `h3_coverash3`)
- `core` — `ARRAY<BIGINT>` cells fully inside the geometry (from `h3_polyfillash3`)
- `holes_cover` — `ARRAY<BIGINT>` cells overlapping holes (empty array if no holes)
- `holes_core` — `ARRAY<BIGINT>` cells inside holes (empty array if no holes)
- `solid_core` — `ARRAY<BIGINT>` cells inside the solid (outer ring, holes filled); used only by `'boundary-in-ignore-holes'`, empty otherwise
- `k` — Ring distance (0 = covering set only)
- `mode` *(optional)* — boundary classification mode (default `'boundary-out'`)
...
**Signature:** `h3_geomkloop(cover, core, holes_cover, holes_core, solid_core, k, [mode])`
...
**Signature:** `h3_geomkringexplode(cover, core, holes_cover, holes_core, solid_core, k, [mode])`
...
**Signature:** `h3_geomkloopexplode(cover, core, holes_cover, holes_core, solid_core, k, [mode])`
