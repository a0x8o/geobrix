...
H3 geometry-aware kring/kloop are **light-tier only** (no Scala/heavy implementation), but they work exactly like the other grids: a single **geometry-taking** function does the whole job in pure Python via the `h3` library — the performant `polygon_to_cells_experimental` polyfill for the covering set and `grid_disk` for the neighbour walk. There is **no dependency on Databricks product functions**, so they run anywhere (local, Serverless, and classic clusters) and are called identically from SQL and PySpark:
...
Geometry-aware H3 k-ring: the geometry's covering set expanded outward by `k` ring steps — the polyfill and neighbour walk both use the `h3` library, with no dependency on Databricks product functions.

**Signature:** `h3_geomkring(geom, resolution, k, [mode])`

**Parameters:**
- `geom` — geometry column (WKB `BINARY` or WKT `STRING`)
- `resolution` — H3 resolution (`0`–`15`)
- `k` — ring distance (`0` = covering set only)
- `mode` *(optional)* — boundary classification mode (default `'boundary-out'`); one of `'boundary-out'`, `'boundary-in'`, `'boundary-in-ignore-holes'`, `'hole-in'`, `'hole-out'`, `'hole-out-ignore-geom'`
...
**Signature:** `h3_geomkloop(geom, resolution, k, [mode])`
...
**Signature:** `h3_geomkringexplode(geom, resolution, k, [mode])`
...
**Signature:** `h3_geomkloopexplode(geom, resolution, k, [mode])`
