...
Product function names (`h3_coverash3`, `h3_polyfillash3`) are confirmed against the Databricks H3 API conventions. Hole extraction (for hole-mode variants) is deferred pending integration verification — hole modes will return empty results for geometries with holes until wiring is complete.
:::

**Dilation modes** work the same as for BNG/quadbin/custom, but hole-mode variants (`'hole-in'`, `'hole-out'`, `'hole-out-ignore-geom'`) require hole extraction which is not yet wired; they return empty results on holed geometries.
