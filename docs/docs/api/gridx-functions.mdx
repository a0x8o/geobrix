...
**Dilation modes** (exact hyphenated values) — each produces a `k`-thick band; polygon holes are handled only by the `hole-*` modes, never by `boundary-*`:

| Mode | Behaviour |
|------|-----------|
| `'boundary-out'` | Default. The geometry's outer **boundary ring** (`k=0`) plus the outward band — cells within `k` steps **outside** the boundary; the geometry's **interior** is excluded. Symmetric with `'boundary-in'` (same `k=0` ring, opposite direction). Works on aligned and non-aligned geometries alike. |
...
**Coverage** (exact values; `coverage` defaults to `'coveras'`):
