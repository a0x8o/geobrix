...
GridX is **partially lightweight** as of v0.4.0: the **quadbin** (`gbx_quadbin_*`) and **BNG** (`gbx_bng_*`) functions run in both tiers <Tier both/>, while the **custom-grid** (`gbx_custom_*`) functions remain heavyweight-only <Tier heavy/>. Each section below is marked with its tier. See [Choosing an Execution Tier](./execution-tiers) for the lightweight vs heavyweight comparison.
