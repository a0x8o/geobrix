...
| Mode | Behaviour |
|------|-----------|
| `'boundary-out'` | Default. The outward band only — cells within `k` steps **outside** the geometry; the geometry's own cells (interior + boundary ring) are excluded. Works on aligned and non-aligned geometries alike. |
