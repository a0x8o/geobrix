...
Most PMTiles functions — the DataSource writer and all options — are available in the **Heavyweight** execution tier only. The aggregate `gbx_pmtiles_agg` is available in **both tiers** (see the per-function badge below). See [Choosing an Execution Tier](./execution-tiers) for the lightweight vs heavyweight comparison.
