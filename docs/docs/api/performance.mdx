...
The quadbin functions are cheap-compute cell math, so the JVM↔Python boundary cost — not the work — sets the lightweight spark-path timing. Vectorizing the cell-id ops into Arrow `pandas_udf`s (crossing that boundary once per batch instead of once per row) makes the lightweight tier **competitive-to-faster across most of the surface**. Benchmarking all 10 functions on a cluster (1,000 rows, 5 measured iterations, both tiers), **6 of 10 are at or above heavyweight parity** — e.g. `gbx_quadbin_aswkb` 1.18×, `gbx_quadbin_kring` 1.14×, `gbx_quadbin_polyfill` 1.11×, `gbx_quadbin_resolution` 1.06×, `gbx_quadbin_distance` 1.02× (the numpy-vectorized encoder closed what had been a ~2.5× per-row-dispatch gap on the cheap cell-id ops). The four still modestly behind (0.85–0.93×) — `centroid`, `tessellate`, `cellunion_agg`, `pointascell` — are the per-row geometry/aggregation shapes, where WKB serialization or shapely union/intersection across the boundary is the cost the JVM tier avoids. Absolute costs are sub-millisecond per row and exact cell-set parity holds across both tiers for all 10. See [Benchmarking — Grid](./benchmarking) for the full table.
