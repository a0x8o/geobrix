...
The quadbin scalar functions register as plain per-row `spark.udf.register` UDFs (not Arrow `pandas_udf`), to keep the package Serverless-safe with no Spark-config or JVM access. They return scalars (`BIGINT`/`INT`/EWKB `BINARY`) or arrays. Because the per-row compute is sub-millisecond, the per-row Python-UDF dispatch crossing the JVM↔Python boundary is the dominant cost relative to the heavyweight JVM tier (which has no such boundary) — see [the perf narrative below](#where-the-lightweight-tier-wins--and-where-it-doesnt). A vectorized (`pandas_udf`) encoder pass is a planned optimization to close that gap.
...
The quadbin functions are cheap-compute cell math, so the cluster spark-path numbers are dominated not by the work but by the **per-row Python-UDF dispatch crossing the JVM↔Python boundary** — overhead the heavyweight JVM tier does not pay. On a cluster (1,000 rows, both tiers) heavy is faster on all four shapes: `gbx_quadbin_tessellate` 0.86×, `gbx_quadbin_cellunion_agg` 0.76×, `gbx_quadbin_polyfill` 0.55×, and the cheapest scalar op `gbx_quadbin_pointascell` 0.40× (≈2.5× slower) — the gap is largest exactly where the per-row compute is smallest, because the dispatch tax then dominates. Absolute costs are sub-millisecond per tile, exact cell-set parity holds across both tiers, and a vectorized (`pandas_udf`) encoder pass is a planned optimization to close the scalar-op gap. The lightweight tier's value here is **Serverless/ARM reach and no-JVM/JAR install**, not raw per-op speed. See [Benchmarking — Grid](./benchmarking) for the full table.
