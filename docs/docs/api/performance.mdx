...
The combine family reduces an array of aligned tiles to one per-pixel statistic. In pure-core (a single tile group) the lightweight NumPy path is competitive — within ~0.5–1.7× of the heavyweight tier, and faster for some (`rst_combinecount`, `rst_combinestddev`). At Spark-path scale (1,000 tile groups on a cluster) the lightweight tier runs ~15–19× slower: each row moves full tile buffers across the JVM↔Python boundary for a scalar Python UDF, and that per-row byte-serialization cost dominates a reduction whose per-pixel compute is cheap. Output parity holds (every compared statistic exact or within tolerance). These functions are correct and production-usable; the heavyweight tier is the better choice for large distributed combine workloads, and an Arrow-batched lightweight path is the tracked optimization. `rst_align_to` shares this tile-byte shape.
