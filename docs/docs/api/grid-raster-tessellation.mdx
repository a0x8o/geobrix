...
- `gbx_rst_h3_rastertogrid{avg,count,max,min,median}(tile, resolution, [coverage], [assignment])` — scalar aggregation
...
- `gbx_rst_quadbin_rastertogrid{avg,count,max,min,median}(tile, resolution, [coverage], [assignment])` — scalar aggregation
...
- `gbx_rst_bng_rastertogrid{avg,count,max,min,median}(tile, resolution, [coverage], [assignment])` — scalar aggregation
...
**Available functions:**
- `gbx_rst_custom_tessellate(tile, grid, resolution, [assignment], [coverage])` — UDTF, one chip per cell
- `gbx_rst_custom_rastertogrid{avg,count,max,min,median}(tile, grid, resolution, [coverage], [assignment])` — scalar aggregation
- `gbx_rst_custom_rasterize_agg(...)` — burns grid cell geometries onto a raster output (inverse direction)
- `gbx_custom_cellfill(cellid, value, grid, [k], [method], [power])` — post-aggregation cell-space fill
