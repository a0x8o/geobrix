...
When the total GeoJSON + PNG payload fits within the 64 MB default embed budget, `plot_interactive` embeds everything inline in the HTML string — no tile server, no separate request. When it doesn't fit, the embed-size ladder picks the right path automatically.
...
| `raster_layer(data)` | File path, bytes, `numpy.ndarray` | Path → rasterio; ndarray → unit-square corners. |
...
`simplify_tiles_from_source` requires the `tippecanoe` binary. The `[vizx]` extra does not install it automatically — install it separately (e.g. `brew install tippecanoe` or `apt-get install tippecanoe`).
:::
