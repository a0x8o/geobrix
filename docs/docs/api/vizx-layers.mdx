...
<figure>
  <img src="../../../resources/images/diagrams/vizx/screenshots/vizx-interactive-hero.png" alt="A multi-layer interactive map rendered by the VizX compositor — self-contained MapLibre, no tile server." style={{width: '100%'}} />
  <figcaption>A multi-layer interactive map rendered by the VizX compositor — self-contained MapLibre, no tile server.</figcaption>
</figure>
...
If you already have a PMTiles archive and only need to trim its zoom range (for example, to cut a 0–14 archive down to 0–8 for an overview embed), use `simplify_tiles_from_archive(path, spec=…)` directly — it operates on the existing archive without re-tiling from source. When the archive exceeds a byte budget set in `spec`, it automatically falls back to a source re-tile.

:::note Requires tippecanoe
`simplify_tiles_from_source` and `simplify_tiles_from_archive` require the `tippecanoe` binary.
