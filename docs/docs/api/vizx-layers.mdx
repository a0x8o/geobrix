...
`plot_interactive` converts each layer to MapLibre GL sources, assembles one self-contained HTML string, and either embeds it inline or streams it from a URL depending on the embed-size ladder below. Use `dry_run=True` to inspect the size audit without rendering.
