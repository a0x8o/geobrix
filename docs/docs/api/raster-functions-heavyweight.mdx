...
# VRT Python Pixel Functions (Heavyweight)

As of v0.4.0, **every RasterX function is available in both execution tiers** — see the [RasterX Function Reference](./raster-functions) for the full list (badged per function, with lightweight-tier notes) and [Choosing an Execution Tier](./execution-tiers) for the comparison. This page documents one heavyweight-tier execution detail: the GDAL VRT Python pixel-function configuration used by `gbx_rst_combineavg` / `gbx_rst_derivedband` (and their `_agg` forms).
