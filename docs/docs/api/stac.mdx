...
- **No `spark.conf.set`.** Parallelism is controlled entirely via `partitions=` in `search` and `download`, and via `DataFrame.repartition()` in your notebook. AQE handles downstream shuffle sizing.
