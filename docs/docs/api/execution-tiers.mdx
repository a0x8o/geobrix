...
The one-line *import* swap is symmetric, but the *install* is not. The **lightweight** tier is just the `[light]` wheel (`%pip`, no JAR, no init script). The **heavyweight** tier (`databricks.labs.gbx.rasterx`) additionally requires the **GeoBrix JAR as a cluster library and the GDAL init script** on a **classic x86 cluster** — the wheel alone will not resolve the `rasterx` import or the JVM `gbx_rst_*` expressions. See [Installation](../installation) for the heavyweight setup.
:::
