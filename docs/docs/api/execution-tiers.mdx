...
The vector OGR readers and grid/vector surfaces (case 3) are the remaining heavyweight-only surfaces. Raster I/O is now available in both tiers; heavyweight's unique surface is expected to keep narrowing.
