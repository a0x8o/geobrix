...
The one-line swap keeps your code identical across tiers. The lightweight tier is functionally complete for raster — it implements every `rst_*` function (all 107).
...
Across the benchmark suite the two tiers agree within tolerance on the large majority of functions — interior values match even where edge handling differs. The one by-design difference is **`rst_convolve`**, which differs slightly at raster edges: the heavyweight tier applies a GDAL block-halo convolution that no single lightweight boundary mode reproduces exactly (interior values still agree). See **[Benchmarking](./benchmarking)** for the latest per-function heavy-vs-light results and how to run the benchmark on a cluster or locally.
