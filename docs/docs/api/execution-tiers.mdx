...
3. **You need GridX, or the heavy-only `conforming` triangulation mode** — the BNG/quadbin/H3 grid APIs (`gbx_bng_*`) are heavyweight-only. The full **VectorX** function set is now available in the lightweight `pyvx` tier — vector-tile encoding (`gbx_st_asmvt`, `gbx_st_asmvt_pyramid`), TIN surface modeling (`gbx_st_triangulate`, `gbx_st_interpolateelevation*`), and legacy-geometry migration (`gbx_st_legacyaswkb`); only the Steiner-point `conforming` triangulation mode is heavyweight-only (the default `constrained` mode runs in both tiers). See [VectorX Function Reference](./vectorx-functions).

The vector OGR readers, GridX, and the `conforming` triangulation mode are the remaining heavyweight-only surfaces. Raster I/O and the full VectorX function set are now available in both tiers; heavyweight's unique surface is expected to keep narrowing.
