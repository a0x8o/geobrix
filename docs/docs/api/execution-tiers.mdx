...
3. **You need GridX, or the VectorX TIN / legacy-geometry functions** — the BNG/quadbin/H3 grid APIs (`gbx_bng_*`), TIN surface modeling (`gbx_st_triangulate`, `gbx_st_interpolateelevation*`), and legacy-geometry migration (`gbx_st_legacyaswkb`) are heavyweight-only. **Vector-tile encoding** (`gbx_st_asmvt`, `gbx_st_asmvt_pyramid`) is available in the lightweight `pyvx` tier — see [VectorX Functions (Lightweight)](./vectorx-functions-lightweight).

The vector OGR readers, GridX, and the TIN/legacy-geometry VectorX functions (case 3) are the remaining heavyweight-only surfaces. Raster I/O and MVT tile encoding are now available in both tiers; heavyweight's unique surface is expected to keep narrowing.
