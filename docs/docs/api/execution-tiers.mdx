...
3. **You need the heavy-only GridX custom grids, or the heavy-only `conforming` triangulation mode** — GridX is now **partially lightweight**: the **quadbin** (`gbx_quadbin_*`) and **BNG** (`gbx_bng_*`) functions run in both tiers via the lightweight `pygx` package, and only the **custom-grid** (`gbx_custom_*`) APIs remain heavyweight-only. The full **VectorX** function set is now available in the lightweight `pyvx` tier — vector-tile encoding (`gbx_st_asmvt`, `gbx_st_asmvt_pyramid`), TIN surface modeling (`gbx_st_triangulate`, `gbx_st_interpolateelevation*`), and legacy-geometry migration (`gbx_st_legacyaswkb`); only the Steiner-point `conforming` triangulation mode is heavyweight-only (the default `constrained` mode runs in both tiers). See [GridX](./gridx-functions) and [VectorX Function Reference](./vectorx-functions).

The vector OGR readers, GridX's custom-grid APIs, the `conforming` triangulation mode, and the heavy `pmtiles` DataSource writer are the remaining heavyweight-only surfaces. Raster I/O, the full VectorX function set, GridX quadbin and BNG, and the `gbx_pmtiles_agg` aggregate are now available in both tiers; heavyweight's unique surface is expected to keep narrowing.
