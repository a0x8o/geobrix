...
:::note Lightweight tier (pyrx)
Powered by **rasterio** + **h3**. Clips the raster to each overlapping H3 cell at `resolution`, returning an `ARRAY<tile>` (one per cell, `cellid` = H3 cell id) — `explode()` for one row per cell. The heavyweight tier emits the cells as separate generator rows.
:::
