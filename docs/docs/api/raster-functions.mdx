...
Both execution tiers alias the module as `rx`, so the example is identical — only the import line differs:
...
| Function | Purpose |
|---|---|
| `rst_maketiles` | subdivide into smaller tiles |
| `rst_retile` | retile to uniform dimensions |
| `rst_tooverlappingtiles` | create overlapping tiles |
| `rst_separatebands` | split multi-band into an array of bands |
