...
### rst_h3_gridspec (Python helper)

:::note Python / DataFrame API only
`rst_h3_gridspec` is **not registered as a SQL function**. It is a Python helper available in both execution tiers as `rx.rst_h3_gridspec(cellids, srid, mode, kring_pad)`.
:::

`rst_h3_gridspec` computes the shared, snapped canvas (extent + pixel size) that a collection of H3 cells should use so that per-threshold rasters align on a common grid and can be stacked via `rst_frombands_agg` or mosaicked via `rst_merge_agg`.

**Typical multi-threshold workflow:**

1. Call `rx.rst_h3_gridspec(cellids, srid=4326, mode='centroids', kring_pad=1)` once on the distinct cell set to get a single `(xmin, ymin, xmax, ymax, width, height)` grid spec.
...
**Parameters:**
- `cellids` — iterable of H3 cell IDs (BIGINT or STRING) that define the canvas
- `srid` — EPSG code; `4326` for WGS 84 (default)
- `mode` — `'centroids'` (default) or `'spatial_envelope'`
- `kring_pad` — ring count by which to expand the canvas boundary (default `1`)

**Returns:** `dict` with keys `xmin`, `ymin`, `xmax`, `ymax`, `width_px`, `height_px`, `pixel_size`
