...
| `/path/to/dir/` | Reads **all** shapefiles found under `dir` recursively, grouped by stem, and unions the results into one DataFrame. All shapefiles must share the same schema; if their schemas diverge, the reader raises an error — load them separately or point at a single-stem directory. |
