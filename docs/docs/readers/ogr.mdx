...
import ogrGbx from '!!raw-loader!../../tests/python/readers/ogr_gbx_examples.py';
...
<TabItem value="lightweight" label="Lightweight · ogr_gbx">

`ogr_gbx` is the lightweight catch-all vector reader (pyogrio-backed, no JAR). It reads any OGR-supported format and emits the same schema as the heavyweight `ogr` reader.

<CodeFromTest code={ogrGbx} language="python" functionName="READ_OGR_GBX"
  source="docs/tests/python/readers/ogr_gbx_examples.py"
  testFile="docs/tests/python/readers/test_ogr_gbx_examples.py" />
