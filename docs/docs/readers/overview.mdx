...
The lightweight tier ships native Python DataSource V2 raster readers — no JAR, no init script. Register them once per session via `pyrx.ds.register`, then use like any Spark reader.
...
| [Lightweight GeoTIFF Reader](./gtiff_gbx) | `gtiff_gbx` | Pure-Python GeoTIFF reader (preset `driver="GTiff"`) |

See the [Lightweight Raster Reader](./raster_gbx) page for full usage and options.
