...
The lightweight tier ingests rasters today with Spark's built-in `binaryFile` reader plus `rx.rst_fromcontent`:

```python
from databricks.labs.gbx.pyrx import functions as rx

df = (spark.read.format("binaryFile").load("/Volumes/.../*.tif")
        .selectExpr("content")
        .select(rx.rst_fromcontent("content", "GTiff").alias("tile")))
```

This approach requires no JAR and no init script — only the `geobrix` Python package installed in the cluster environment.

Dedicated `*_pyrx` readers — e.g. `spark.read.format("gtiff_pyrx")` — built on the [PySpark Python Data Source API](https://spark.apache.org/docs/latest/api/python/tutorial/sql/python_data_source.html) (no JAR, no init script) are planned for a future release.
