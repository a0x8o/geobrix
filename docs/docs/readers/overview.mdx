...
features by `chunkSize`, raster readers split large files by `sizeInMB` — and the result is
...
| [Vector Reader](./ogr) | `ogr_gbx` | Pure-Python catch-all vector reader (pyogrio; same OGR schema) |
...
counterpart. The generic `ogr_gbx` catch-all reader pairs with the heavyweight `ogr` generic
...
↔ `gpkg_ogr`, `file_gdb_gbx` ↔ `file_gdb_ogr`. The **generic catch-all** pair is `ogr_gbx`
