...
File Geodatabases contain multiple feature classes (layers); both tiers expose a layer selector. The OpenFileGDB driver is preset on both named readers.
...
- `layerNumber` - Feature-class index to read (default: `"0"`, first feature class).
- `layerName` - Feature-class name to read; takes precedence over `layerNumber` when set.
