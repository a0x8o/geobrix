...
The OpenFileGDB driver is preset.
...
| `layerNumber` | `"0"` | Feature-class index to read (0-based, first feature class). |
...
| Option | Default | Description |
|--------|---------|-------------|
| `layerName` | `""` (first feature class) | Feature-class (layer) name to read; takes precedence over `layerN`. |
| `layerN` | `"0"` | Feature-class index to read (0-based). |
