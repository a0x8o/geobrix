...
|--------|---------|-------------|
| `driverName` | _required_ | OGR driver name (e.g. `GPKG`, `ESRI Shapefile`). Required on the generic `vector_gbx`; preset on the named `*_gbx` readers. |
...
| `layerNumber` | `"0"` | Layer index for multi-layer formats (0-based). |
| `layerName` | `""` | Layer name for multi-layer formats (overrides `layerNumber`). |

### Heavyweight (`ogr`)

| Option | Default | Description |
|--------|---------|-------------|
| `driverName` | auto-detected from extension | Force a specific OGR driver regardless of file extension (e.g. a `.txt` that is really GeoJSON). |
