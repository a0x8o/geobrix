...
:::note Hybrid — requires the native GDAL
Writing a File Geodatabase needs the native GDAL Python bindings (`osgeo`) from the
heavyweight GDAL init script — pyogrio's bundled GDAL has a read-only OpenFileGDB driver.
On compute with those natives, `file_gdb_gbx` writes the `.gdb` natively; without them it
raises a clear error (use `gpkg_gbx` / `geojson_gbx` instead). FileGDB **reading** is
lightweight-only (pyogrio reads it). The output path must end in `.gdb`.
:::
...
`file_gdb_gbx` is a hybrid (it encodes via the native GDAL `osgeo`); see the note above.
:::
