...
| `fileName` | _(none)_ | Name the output unit explicitly. When set, `.save(path)` treats `path` as the **parent directory** (created if missing) and writes `path/<fileName>` with the extension auto-completed (`.shp.zip` when `zip=true`, otherwise a sidecar directory `path/<fileName>/`). See [Output naming](#output-naming). |
