...
:::tip Writing a large Shapefile? Read this first
A Shapefile assembles on a single node, and the format hard-caps each
`.shp`/`.dbf` at **2 GB**. For large outputs, see [**Choosing a writer for large
datasets**](./overview#choosing-a-writer-for-large-datasets) in the Writers
Overview — it explains the driver-memory ceiling (use a **classic cluster**;
high-memory Serverless does *not* help here) and the scalable `geojsonl_gbx` /
`gpkg_gbx` alternatives.
:::

:::note Single-file write is lightweight-only
...
[**GeoJSONL** writer](./geojsonl) (`geojsonl_gbx` lightweight / `geojsonl` heavyweight)
...
).write.format("shapefile_gbx").mode("overwrite").save("/Volumes/cat/sch/vol/out")
```
...
**Output:** an ESRI Shapefile — a `.shp` plus its `.shx` / `.dbf` / `.prj` sidecars. Note the Shapefile format constraints: **attribute field names are truncated to 10 characters**, attribute types are limited by the dBASE (`.dbf`) format, and a file holds a single geometry type.
