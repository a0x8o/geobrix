...
- **`shapefile_gbx`** — the format caps each `.shp`/`.dbf` at **2 GB**. Practical up
  to roughly a few hundred thousand features with attributes; beyond that the `.dbf`
  (one fixed-width record per row) is the usual limit.
