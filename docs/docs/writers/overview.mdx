...
The **single-file** writers assemble the whole output on the **driver** (a single
node), so they have scale ceilings:

- **`shapefile_gbx`** — the whole file is assembled on the driver, and the format
  hard-caps each `.shp`/`.dbf` at **2 GB**. Large attribute tables (the `.dbf` is
  one fixed-width record per row) make the single-node write the practical limit.
- **`geojson_gbx`** — merges every partition into one FeatureCollection on the
  driver, so it is bounded by **driver memory**.

For large datasets, prefer the writers that don't assemble on one node:

- **`geojsonl_gbx`** — writes one shard per partition (no driver merge); throughput
  scales with partitions. The most scalable option.
- **`gpkg_gbx`** — a single file with **no 2 GB cap**; features are streamed per
  partition, so it handles far larger data than shapefile while staying one file.
