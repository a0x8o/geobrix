...
A tile pyramid: one row per tile, `bytes` being the already-encoded tile payload (PNG/JPEG/WebP/MVT/…). The writer requires exactly these four columns — extra or missing columns raise an error. Coerce your DataFrame to this shape (and column order) first:

```python
tiles.select(
    tiles["zoom"].alias("z"),
    tiles["tile_x"].alias("x"),
    tiles["tile_y"].alias("y"),
