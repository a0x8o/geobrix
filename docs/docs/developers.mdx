...
| Command | When to use | What it does |
|---------|-------------|--------------|
| `gbx:ci:push` | Initiate remote build on current branch (e.g. beta/0.3.0) | Pushes branch to origin, then watches the **build main** workflow run |
...
Pushing to a branch (except `python/**` and `scala/**`) **successfully triggers** the **build main** workflow. To run the main build on your current branch (e.g. `beta/0.3.0`):
