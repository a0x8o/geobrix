...
A three-notebook series that takes one San Francisco bounding box and turns it into three fully self-contained [PMTiles](https://protomaps.com/docs/pmtiles) archives — one per data modality — using GeoBrix on Databricks.

The notebooks follow a solar site-selection narrative: identify candidate rooftops (vector, NB01), overlay an aerial basemap for visual validation (raster, NB02), and layer terrain-derived slope, aspect, and hillshade to score each candidate by sun exposure (elevation, NB03). All three archives feed a single `plot_pmtiles` / `plot_cog` viewer at the end, giving a single-browser-tab deliverable with no tile server.
...
:::tip View on GitHub
**[notebooks/examples/helios](https://github.com/databrickslabs/geobrix/tree/main/notebooks/examples/helios)** — download the folder and import `config_nb.ipynb` and the three numbered notebooks into your Databricks workspace to run.
:::
...
- **GeoBrix PMTiles** (`pyrx` / `pyvx` / SQL): [`gbx_pmtiles_agg`](../api/pmtiles-functions#pmtiles_agg) — aggregate tile bytes into a single archive; [PMTiles Writer](../writers/pmtiles) (`.write.format("pmtiles_gbx")`) — for larger pyramids.
