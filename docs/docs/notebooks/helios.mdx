...
The notebooks ship with `INTERACTIVE_PLOTS = False` so the committed `.ipynb` renders fast, GitHub-compatible static maps. Set `INTERACTIVE_PLOTS = True` in `config_nb` or in a cell right after `%run ./config_nb` for interactive MapLibre maps.
:::
