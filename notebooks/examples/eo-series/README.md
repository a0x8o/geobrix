...
> __Lightweight tier (Serverless) by default.__ The series uses the lightweight tier — pure Python/PySpark bindings (`databricks.labs.gbx.pyrx`) plus the `geobrix[light,stac]` wheel, installed by `config_nb` — so it runs on Serverless with no JAR. STAC search and download are handled by `StacClient` from `databricks.labs.gbx.stac` (see [STAC Client](https://databrickslabs.github.io/geobrix/docs/api/stac)). To run heavyweight instead, flip the commented *option-2* (`rasterx`) in `config_nb.ipynb` **and** `library.py`, and attach the GeoBrix JAR + [GDAL init script](https://databrickslabs.github.io/geobrix/docs/installation) to a classic x86 cluster. Spark-conf tuning goes through a `set_conf_safe()` helper that no-ops on Serverless. See [Execution Tiers](https://databrickslabs.github.io/geobrix/docs/api/execution-tiers).
...
| File | Purpose |
|---|---|
| `config_nb.ipynb` | Shared setup (`%run ./config_nb` from every main notebook). Installs the `geobrix[light,stac]` wheel + EO deps, selects the tier (option-1 `pyrx` default / option-2 `rasterx`), registers functions + light readers/writers, sets Unity Catalog `catalog_name` / `schema_name`, creates the `/Volumes/<cat>/<schema>/data/alaska` ETL tree, exposes the `FORCE_REBUILD` toggle and the Serverless-safe `set_conf_safe()` helper, instantiates `stac_client = StacClient()`, and defines tiling helpers (`finalize_tiled_band_tbl`, `gen_tessellate_tiled_band`, viz helpers `as_gdf` / `cells_as_gdf`). |
| `library.py` | Python module (not a notebook) with reusable functions imported from `config_nb`: raster/rasterio plotting helpers (`plot_raster`, `plot_file`, `rasterio_lambda`, `to_numpy_arr`). |
...
- **GeoBrix** (version 0.4.0). `config_nb.ipynb` `%pip`-installs the `geobrix[light,stac]` wheel — pure-Python bindings + rasterio + the STAC client dependencies (`pystac-client`, `planetary-computer`, `tenacity`, `requests`) — nothing is assumed pre-staged. For the heavyweight tweak, flip option-2 (`rasterx`) in `config_nb.ipynb` **and** `library.py`, and attach the GeoBrix JAR + GDAL init script to the cluster.
